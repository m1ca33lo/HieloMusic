...
const command = new SlashCommand()
  .setName("play")
  .setDescription(
    "Pide a Hielo que busque tu canción favorita en YouTube, spotify, deezer y Apple Music."
  )
  .addStringOption((option) =>
    option
      .setName("song")
      .setDescription("¿que estoy buscando?")
  ...
  .setRun(async (client, interaction, options) => {
    ...
    if (!node) {
      return interaction.reply({
        embeds: [client.ErrorEmbed("El Reproductor no esta conectado")],
      });
    }
    ...
    const ret = await interaction.reply({
      embeds: [
        new MessageEmbed()
          ...
          .setImage(imagenes.buscando),
      ],
      fetchReply: true,
    });

    let song = options.getString("song", true);
    let res = await player.search(song, interaction.user).catch((err) => {
      client.error(err);
      return {
    ...
    if (res.loadType === "TRACK_LOADED" || res.loadType === "SEARCH_RESULT") {
      ...
      let addQueueEmbed = new MessageEmbed()
        .setColor(client.config.embedColor)
        .setAuthor({ name: "Agregado a la cola", iconURL: imagenes.playdj })
    ...
    if (res.loadType === "PLAYLIST_LOADED") {
      ...
      let playlistEmbed = new MessageEmbed()
        ...
        .setAuthor({
          ...
          iconURL: imagenes.playdj,
        })
        .setThumbnail(res.tracks[0].thumbnail)
        .setDescription(`[${res.playlist.name}](${song})`)
        .addFields(
          {
            name: "En cola",
